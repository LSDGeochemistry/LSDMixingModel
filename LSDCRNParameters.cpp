...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDCRNParameters::scale_F_values(vector<bool> nuclides_for_scaling, double lat, double lon, double site_elev, double Fsp)
{
  // first check the boolean vector. If it is an incorrect size, print a warning
  // and then default to all true
  if(nuclides_for_scaling.size() != 4)
...
// stdatm.m (general use) and antatm.m (Antarctica).
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
double LSDCRNParameters::stone2000sp(double lat, double lon, double site_elev, double Fsp)
{
